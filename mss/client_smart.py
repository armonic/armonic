...
2) If this provide requires other provides (local or external),
they are called.
...
2.2) We fill Requires of Mysql.addDatabase with (in order):
     ...
     1) Values specified by require Mysql.addDatabase of provide
     Wordpress.get_site
...
class Provide(ShowAble):
    ...
    def __init__(self,
        ...
        - Values specified by the require that calls this provide are available
        in self.suggested_args
        -
    ...
    def handle_call(self):
        ...
        :rtype: bool. Return True if the provide must be called, false
        otherwise.
        """
        self.show("Preparing the call of provide %s.%s..." %
                  (self.lf_name, self.provide_name))
        msg = "Where do you want to call it?"
        ret = user_input_variable(prefix=self.sep(),
                                  variable_name='host',
                                  message=msg,
                                  prefill=self.caller_provide.host)
        host = ret['host']
        self.host = host
    ...
    def get_requires(self):
        ...
        self.provide_call_requires = self.lf_manager.call(
                                            "provide_call_requires",
                                            self.lf_name, self.provide_name)
    ...
    def call(self):
        if self.handle_call():
            ...
            self.show("Provide '%s' will be called with Requires:" %
                      (self.provide_name))
...
class Require(object):
    ...
    - Default values of requires of this provide :
        ~provide_requires[*].defaults
    """
    ...
    def _build_validate(self, values):
        while True:
            try:
                ...
                values = self.on_require_not_filled_error(e.variable_name,
                                                          values)
                continue
...
class RequireSimple(mss.require.Require, ShowAble):
    def _build(self, provide_caller):
        ...
        suggested = dict([(s.name, s.value) for s in
                          provide_caller.suggested_args])
        ret = update_empty(needed, suggested)

        self.fill(ret)
        self.validate()
        logger.debug("Require %s of provide %s has be filled with :" %
                     (self.name, provide_caller.provide_name))
        logger.debug(ret)
...
class RequireUser(mss.require.RequireUser, Require, ShowAble):
    ...
    def build_save_variables(self):
        """Redefine it to save require variables."""
        Variables.append((self.variables[0].full_name,
...
class RequireLocal(mss.require.RequireLocal, RequireWithProvide, ShowAble):
    def on_require_not_filled_error(self, err_variable, values):
        ...
        values.update(user_input_variable(variable_name=err_variable,
                                          message=msg,
                                          prefix=self.sep(),
...
p = Provide(lf_name='Wordpress',
            provide_name='get_site',
            caller_provide=ProvideUser())
p.call()
