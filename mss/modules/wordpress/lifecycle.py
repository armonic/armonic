...
class Configured(State):
    ...
    def entry(self,requires):
        ...
        logger.info("%s.%-10s: edit php wordpress configuration file with %s"%(self.module(),self.name,requires))
...
class Active(State):
    ...
    def entry(self,requires):
        logger.info("%s.%-10s: activation with %s"%(self.module(),self.name,requires))
        self.httpdDocumentRoot=requires['Httpd.get_documentRoot'][0]['httpdDocumentRoot']
        logger.info("%s.%-10s: TODO : write to MSS database : wordpress use a vhost=%s"%(self.module(),self.name,requires['Httpd.get_documentRoot'][0]['httpdDocumentRoot']))
    def leave(self):
