...
class Directive(Node):
    label="directive"
    def xpath_access(self):return "%s[. = '%s']" %(self.label,self.value)
...
class Directory(Node):
    ...
    def children(self):return [self.path,
                               self.allowOverride,
                               self.option,
                               self.order]
...
#    directories=Directories
    port=Child("port",baseXpath="//*[label() != IfModule[*]]/directive[. = 'Listen']",label="arg")
    portVhost=Child("portVhost",baseXpath="/files/etc/apache2/sites-available/default/VirtualHost",label="arg")
#    documentRoot=Child("documentRoot",baseXpath="/files/etc/apache2/sites-available/default/VirtualHost/directive[. = 'DocumentRoot']",label="arg") # DEBIAN ONLY
    documentRoot=Child("documentRoot",baseXpath="//*[label() != 'default-ssl']/*/*/directive[. = 'DocumentRoot']",label="arg")
    
    lenses={"Httpd":["/etc/apache2/ports.conf","/etc/apache2/apache2.conf","/etc/apache2/sites-available/*",
